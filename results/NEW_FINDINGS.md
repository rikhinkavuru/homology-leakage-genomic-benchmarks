...
Cluster bootstrap over within-test 8-mer-Jaccard>0.7 components. Both leaky datasets: CIs widen ×1.0–1.8, **no verdict flips**. RF drops still exclude 0 (nontata delta CI [0.092,0.126]; ensembl [0.155,0.173] -- both from `cluster_bootstrap_full.csv`); ensembl-LR stays null both ways. Design effect small because test-internal clustering ≈1.06–1.19 seq/cluster (near-dups are test-to-**train**, not test-to-test). → R3.3 answered; the correct tool changes nothing. (Full grid + ICC/deff/CRVE/combined-source CI: `exp_clusterboot_full.py`, pending.)
