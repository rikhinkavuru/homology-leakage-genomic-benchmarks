...
# fig_controls headline bars use the reconciled convention (.predict()/argmax, 5-seed-mean
# corrected) so the plotted drops match the text (nontata 11.8 pts, ensembl 16.5 pts).
for _d, _orig, _hom in [("human_nontata_promoters", 0.9284, 0.8101),
                        ("human_enhancers_ensembl", 0.8596, 0.6951)]:
    summ.loc[_d, "original_acc"] = _orig
    summ.loc[_d, "homology_acc"] = _hom
...
ax.set_ylim(0, 118)
# y-floor at 0 (honest scale); extra headroom so the single-row legend clears the bars.
...
ax.legend(fontsize=8.5, frameon=False, loc="upper center", ncol=3,
          bbox_to_anchor=(0.5, 1.0), columnspacing=1.3, handletextpad=0.5)
