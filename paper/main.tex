...
%% ===== AUTHORS / AFFILIATIONS: placeholders for the corresponding author to fill =====
...
%% MISMATCH/NOTE [task-tagged "verify S9: 15.6"]: the 15.6-point figure (used in the
%% abstract and Section 4.2) is the ensembl RandomForest k=6 homology drop from the
%% PRIMARY frozen run -- PAPER_NUMBERS.md S3 (main results), S4 (capacity scaling) and
%% S5 (robustness) all give 0.1558 = 15.6 pts. The separate 4-model extended-capacity
%% table (PAPER_NUMBERS.md S9) lists 0.164 (16.4 pts) for the same cell, a documented
%% RandomForest load-order variance between run paths; 15.6 is the canonical/headline
%% value and is used here.
\abstract{
...
\textbf{Availability and implementation:} Code, data-processing scripts, and the splitter are available at \href{https://github.com/PLACEHOLDER/homology-leakage-audit}{[repository URL --- placeholder]}; the audit is CPU-only and fully reproducible.}
...
%% 15.6 = ensembl RF k=6 drop (primary frozen run); see the MISMATCH/NOTE above the abstract.
